...
export class FileService {
  ...
  private formatOrcsCard(card: OrcsCard): string {
    const lines = [
      ...
      '=== ORCS FORMAT VERSION: 2025.004 ===',
      'UUID: ' + card.id,
      'TITLE: ' + card.title,
      'SOURCE: ' + card.source,
      'SOURCE_HASH: ' + card.sourceHash,
      'SOURCE_REFERENCE: ' + (card.sourceReference || ''),
      ...
      card.content,
      '',
      '=== END HANDLING: ' + (card.handling.length > 0 ? card.handling[card.handling.length - 1] : '') + ' ===',
      '=== END ORCS FORMAT VERSION: 2025.004 ===',
