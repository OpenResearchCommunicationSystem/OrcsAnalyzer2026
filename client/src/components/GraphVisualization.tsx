...
export function GraphVisualization({ onNodeClick }: GraphVisualizationProps) {
  ...
  const renderGraph = (data: GraphData, layoutType: LayoutType) => {
    ...
    try {
      ...
      // Convert data to D3 format with validation
      const nodes: D3Node[] = data.nodes
        .filter(d => d.id && d.label && d.type)
        .map(d => ({
          id: d.id,
          label: d.label,
          type: d.type,
      ...
      if (layoutType === 'hierarchy') {
        ...
        otherNodes.forEach((node, i) => {
          ...
          const nodeRadius = radius + (Math.random() * 50 - 25);
      ...
      node.append("circle")
        .attr("r", (d: any) => d.type === 'entity' ? 15 : 12)
        .attr("fill", (d: any) => {
          switch (d.type) {
      ...
      node.append("text")
        .attr("dx", 0)
        .attr("dy", (d: any) => d.type === 'entity' ? 30 : 25)
        .attr("fill", "#E5E7EB")
        .attr("font-size", "11px")
        .attr("text-anchor", "middle")
        .attr("font-weight", (d: any) => d.type === 'entity' ? "bold" : "normal")
